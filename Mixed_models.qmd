...
$$Cov(Y_{ijt}, Y_{i'j't'}) = \left\{ 
  ...
  σ^2ρ^{|t-t'|} \ \ si \ \ (i, j) = (i', j') \\
     0 \ sinon \
   \end{array}
  \right.$$ Cette structure suppose que les mesures faites sur des animaux diérents sont indépendantes.
On suppose de plus que \|ρ\| \< 1, ce qui implique que celles faites sur un même animal sont d'autant moins corrélées que l'intervalle de temps est grand.
Cette forme de covariance correspond à un processus auto-régressif d'ordre 1, généralement noté AR(1).
...
*Ecriture matricielle* Comme dans le cas des mesures répétées, il n'existe pas d'écriture simple en termes d'effets aléatoires.
De plus, comme toutes les mesures sont dépendantes, la matrice Σ n'est plus diagonale par bloc et s'écrit :
...
>>>>>>> 4a005b60a7bfc23afd1421e0a74fb24cb006139d
set.seed(3)  
n <- 100
