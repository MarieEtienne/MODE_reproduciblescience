...
**Species factor**\
- $species_{Chin}$ = 0 (the baseline of the factor Habitat)\
- $Atm_{Wet}$ = $0.216^{NS}$

**Period factor**\
- $Time_{1}$ = 0 (the baseline of the factor Habitat)\
- $Time_{2}$ = $1.40^{***}$\
- $Time_{3}$ = $0.86^{***}$\
- $Time_{4}$ = $1.27^{***}$

\*\*Interaction\*\
- $Atm_{Wet}$:$Time_{2}$ = $-1.71^{***}$\
- $Atm_{Wet}$:$Time_{3}$ = $0.13^{NS}$\
- $Atm_{Wet}$:$Time_{4}$ = $0.73^{NS}$