...
$$Cov(Y_{ijt}, Y_{i'j't'}) = \left\{ 
  ...
  σ^2ρ^{|t-t'|} \ \ si \ \ (i, j) = (i', j') \\
   ...
   \end{array}
  \right.$$
Cette structure suppose que les mesures faites sur des animaux diérents sont indépendantes. On suppose de plus que |ρ| < 1, ce qui implique que celles faites sur un même animal sont d'autant moins corrélées que l'intervalle de temps est grand. Cette forme de covariance correspond à un processus auto-régressif d'ordre 1, généralement noté AR(1).
