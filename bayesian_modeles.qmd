...
For this example we are going to use data taken from a normal law.So we will the mean but and the standard deviation like we did just before; For this example we will use the software *JAGS* with the library *rjgas*. *JAGS* is a software that let's us build our own model to estimate parameters that we want to find. 
