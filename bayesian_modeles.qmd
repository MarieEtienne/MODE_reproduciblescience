...
Bayesian analyses differ from frequentist analyses by incorporating prior information via conditional probabilities, known as Bayes’ rule (Nathan P. Lemoine, 2019):
$$\frac{[\theta]\cdot[Y\mid\theta]}{[Y]}=[\theta\mid Y]$$
With Pr(θ) = the probability of the parameter or hypothesis θ based on prior information
Pr(Y|θ) = the likelihood of the data conditioned on the hypothesis
Pr(Y) = the normalization constant
Pr(θ|Y) = the posterior probability of the of the hypothesis conditioned on the observed data. 
...
priors <- ggplot(data, aes(x = x, y = y)) + geom_line(color = "cornflowerblue", linewidth = 1) +
  geom_hline(yintercept = 0.05, color = "purple", size = 1) + theme_classic() + labs(x = "Survival", y = "Probability density", title = "Comparaison informative and non-informative priors") + scale_color_manual(name = "Legend", label = c("With prior info"), values = c("Line 1" = "blue", "Line 2" = "red"))
priors
...
This list contains all information needed to make the model run, nothing really interesting to say here.
