...
prior_uniform <- rep(0.05, length(theta))
posterior_uniform <- likelihood * prior_uniform * 12
# Create data frame for plotting
df_uniform <- data.frame(theta, likelihood, prior_uniform, posterior_uniform)
# Plotting the graph 
graph1 <- ggplot(data = df_uniform, aes(x = theta)) +
  geom_line(aes(y = likelihood, color = "Likelihood"), linetype = "dashed", size = 1) +
  geom_line(aes(y = prior_uniform, color = "Posterior"), size = 1) +
  geom_line(aes(y = posterior_uniform, color = "Prior"), size = 1) +
...
# Define the range of theta values
theta <- seq(0.25, 1, by = 0.01)
# Define the number of trials and successes
n <- 10
x <- 9
# Calculate likelihood, prior, and posterior for highly informative prior
likelihood <- choose(n, x) * theta^x * (1 - theta)^(n - x)
prior_informative <- dbeta(theta, 10, 9)
posterior_informative <- likelihood * prior_informative
# Rescale distributions to position them as desired
max_likelihood <- max(likelihood)
max_prior <- max(prior_informative)
max_posterior <- max(posterior_informative)
# Scale factors for prior and posterior to position them below likelihood
scale_factor_prior <- 0.9
scale_factor_posterior <- 0.9
# Rescale prior and posterior
prior_rescaled <- prior_informative * (max_likelihood / max_prior) * scale_factor_prior
posterior_rescaled <- posterior_informative * (max_likelihood / max_posterior) * scale_factor_posterior
# Create data frame for plotting
df_informative <- data.frame(theta, likelihood, prior_rescaled, posterior_rescaled)
# Plotting the graph 
graph2 <- ggplot(data = df_informative, aes(x = theta)) +
  geom_line(aes(y = likelihood, color = "Likelihood"), linetype = "dashed", size = 1) +
  geom_line(aes(y = prior_rescaled, color = "Prior"), size = 1) +
  geom_line(aes(y = posterior_rescaled, color = "Posterior"), size = 1) +
...
Here we illustrate the result of updating the prior with the likelihood, it gives us the posterior distribution. This shows that the impact of the chosen prior influences the posterior distribution with both its shape and the quantity of information it holds compared to the information within the dataset.
