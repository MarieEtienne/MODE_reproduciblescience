...
For this example we will use the software *JAGS* with the library *rjgas*. *JAGS* is a software that let's us build our own model to estimate parameters that we want to find. It need 3 main componente.
Fist, the model :
```{r}
mod1 = "
  model{
...
data_list2 = list(
  Y3 = data$Y3,
  n_obs= length(data$Y3),
  IDclass = data$IDclass,
  n_class = max(data$IDclass)
)
...
for( i in 1:5){
  ...
  if(substr(params_names[5],1,1) == "m"){
...
postSamples_df2 %>%
  ggplot() +
  facet_wrap(~Parameter, scales = "free") +
  geom_histogram(aes(x= value, y =..density.., fill = as.factor(Chain), col = as.factor(Chain)),
                 position = "identity", alpha = 0.3)+
    geom_histogram(aes(x= value, y =..density.., col = as.factor(Chain)),
