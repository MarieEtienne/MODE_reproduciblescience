...
An extra parameter : WX$\theta$, called the autocovariate is added to the generalized linear model's formula. It is a distance-weighted function of the neighboring response value.

**Y = X**$\beta$ + **WX**$\theta$ + $\epsilon$

The argument WX$\theta$ is the average of the neigbouring Xs. 

##### Application in R

As this analysis is quite simple the average value of the neighbouring regions can be calculated by hand and then added to the generalized linear formula.
...
*(The dataset "div35" set can be found in the doc called "Saptial autocorrelation_modelisation")*
...
summary(lmsSLX)
```
Here, 2 coeffiscients are given as well as their p-value. 
The first one (rich_pp) correspond to the argument X$\beta$ of our original function. The second one (lag.rich_pp) correspond to WX$\theta$.

*Here there is only one X, and therefore only 2 coefficients, but with a bigger model to test, the coeffiscients corresponding to the X will be named just like the original Xs, and the coeffiscients for the autocorrelation will be named "lag.X".*

As always, we only keep the coefficients that have a significant p-value.
Here they all do. 
Our final model here would be : 

$Bird\:Richness = -7.40 + 0.47\:Plant\:Richness + 1.19\:Plant\:Richness\:Corr + \epsilon$

An interpretation of those results here is that the more plants there is in a place, the more bird will be present as well. As for the lag value, we can excpect that the more plants there will in the neighbouring regions, the more birds there will in our region.
...
The term $\lambda$W represent the spatial structure with $\lambda$ being the spatial autoregression coefiscient. $\mu$ is the spatially dependant error term.
...
It can be noted that this model can also be used in both situation described previously, as the coeffiscient will cancel themselves out if either one is not present.
