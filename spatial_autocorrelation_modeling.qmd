...
An extra parameter : WX$\theta$, called the autocovariate is added to the generalized linear model's formula.
...
*(For the purpose of the exemple, we will assume that the analysis is the right one even though other would be more fitted)*
...
Depending on where this process is thought to be,three models are availble :
...
$Bird\:Richness = 0.37\:WY +0.53\:X + \epsilon$
...
**Y =** $\rho$**WY +X**$\beta$ + $\lambda$**WY** + $\epsilon$
...
$Bird\:Richness = 0.64WY + 0.53X - 0.65$
...
Even though three  commands are available for each type of SAR model, it is best to only use the mixed model function sacsarlm, as the other ones don't provide p-values, and this one provides more informations than the others. 
