...
The autocorrelation can be detected in a **non-stationnary process**.

**Stationnarity** is the constistency of the statistical parameters of a series such as the mean and the variance (Figure). 

- In non-stationnary processes, these parameters vary in time.
- In stationnary processes, the values gravitate around a mean value within the same range. 

![Stationary and non-stationary processes](C:/Users\User\OneDrive\Bureau\MODE\M2\OCR\Stationarycomparison.png)

Given the precedent plots displayed, one would think there are both stationnary and non-stationnary processes within the different variables.
Tests wil allow us to know with more details if there is a dependency. 
...
To deal with stationnary processes, it is possible to use GLS (Generalized Leat Square) models. 
...
The use of this model appears to have diminished the effect of time dependency.
