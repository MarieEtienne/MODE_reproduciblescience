...
In ecology, a lot of processes are non stationary. This model is one of the few that allow to deal with non-stationarity. It is a local modeling, and will therefore require to perform a regression for each points and its neighbors.
...
**Phi**: kernel bandwidth **RMSPE**: root square of error of the prediction based on bandwidth estimated **Beta**: matrix with the coefficients for each point (intercept line \[1,\] and regression coefficient line \[2,\]) **Yhat**: model estimate **RMSE**: root square of error estimation **Rsquare**: model fitting
