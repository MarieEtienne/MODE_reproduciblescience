...
<<<<<<< HEAD
=======
>>>>>>> ef4b68f34a715a34e1ba68b220d2f0d7b3c0ea3c
...
![](Capture d’écran 2023-11-15 à 16.45.12.png){fig-align="center"
width="190"}

![](Capture d’écran 2023-11-15 à 16.45.20.png){fig-align="center"
width="191"}
...
In addition to representing the variance of the principal components,
...
contribution of each principal component $v_k$ to the overall
information.
...
probabilities of the columns $(f_{.j})$ and rows $(f_{i.})$ are
...
-   Row profiles
    ...
    The row profile is
    $L_i = (\frac{f_{i1}}{f_{i.}},...,\frac{f_{ij}}{f_{i.}})$. In matrix
    terms, we will set up $D_n=diag(f_{i.})$ such that $L = D_n^{-1}F$.
    We can then define the average row profile, obtained by summing the
...
-   **Column profiles**
    ...
    $C_j=(\frac{f_{1j}}{f_{.j}},...,\frac{f_{ij}}{f_{.j}})$. As with row
    profiles, it can be expressed and calculated in matrix terms by
    setting: $D_p=diag(f_{.j})$, such that $C=D_p^{-1}F^T$. We can then
...
by$$d^2(i,i')=\sum_{j=1}^p(\frac{f_{i j}}{f_{i.}}-\frac{f_{i'j}}{f_{i'.}})^2$$
...
*With* $\varphi_j$ : *the* eigenvector *associated with the eigenvalue*
$\lambda_j$ *of the matrix F;* $\Psi_j$ : *the* modality *coordinates of
the row variable in the row space defined by the eigenvalues;*
$\hat{\Psi}_j$ *the* contrinution *of the component* $j$ *to the row
variable in the row space and* $\hat{\varphi}_j$ *the* contrinution *of
the component* $j$ *to the column variable in the column space*

establish an almost barycentric link between the two types of analysis,
meaning that the coordinates of points in one space are proportional to
the components of the factor from the other space corresponding to the
same eigenvalue.
