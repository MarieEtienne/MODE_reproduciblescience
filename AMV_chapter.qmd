...
#df <- read.table("https://gist.githubusercontent.com/slopp/ce3b90b9168f2f921784de84#fa445651/raw/4ecf3041f0ed4913e7c230758733948bc561f434/penguins.csv", sep = "," , #header = TRUE)


###Autres données
Oak <- read.csv("C:/Users/bbong/Documents/cours/M2_EFCE/OCR/Oak_data.csv", sep=';', header = TRUE) 
#Oak_spp <- read.csv("Oak_species.csv", header = TRUE)
...
So here, the first table ("Oak_sp") represents a table of spider abundance for 47 sites (47 rows). We have 56 differents species. The second table ("Topo") corresponds to 4 topographics variables (Latitute, Longitute of sites, the elevation and the slope). As it is not the same unit for each variable we have centred-reduced these four variables.

As explained above, the CCA objective is to link the abundance of spider by the topographics variables. Here, we have *a priori* hypothesis on the effect of these environmental variables on the spiders.
...
An we already have completed our analyse, it was no difficult!
But now, we need analyse the result and test the significant.
...
Finally, the results of the CCA are plot in the distribution law calculated by the permutation. So we can conclude that our environmental variables explain a part of the spider distribution.
...
```
The first output (by *ADE4*) allows us to see each part of the analyse that could be use to describe the result. We will use especially the last part afterward. You also have, at the beginning, the number of rank that corresponds to the number of eigenvalue with their score below.
We can calculate the pourcentage of variance explained by topographic variables.

```{r}
ccavegan
```
Here, it is the output with the *vegan* package.We can see the total inertia in the CCA analyse (5.55) and the inertia explained by the topographic variables (0.71) and by the residuals (4.85). We can therefore conclude that our topographic variables explain only 13% of the spider variances (same result that before).
Below that, we have the eigenvalues for the constrained axes (variance part explained by topographic variables) and for the unconstrained axes (variance part explained by the residuals)
...
plot(ccaoak)
```
Détailler rapidement les différentes parties du plot.

But this can also be decomposed in different plots and output.
