...
Then, we can perform the PCA on the CA that corresponds to the CCA:
```{r}
ccaoak <- pcaiv(coafau,Topo,scannf=F,nf=2)
```
...
Finally, the results of the RLQ are plot in the distribution law calculated by the permutation. So we can conclude that our environmental variables explain a part of the spider distribution.
