...
To be abme to conclude with your results you have to validate your model. All general linear model (linear regression, ANOVA, ANCOVA) need to verify 3 criterions.
...
```
As you can see, the residual are lie mostly the red line so we can say the residuals are normally distributed

A FAIIIIIIIIIIIRE: dire quoi faire si pas normal --> F plutot resistant au écart à la normalité

- *homogeneity of variances*
The assumption of homogeneity of variance, namely that the variation in the residuals is approximately equal across the range of the predictor variables, can be checked by plotting the residuals against the fitted values and the residuals against the significant main effects.
...
# residuals against Water Content
...
abline(h = 0)
```
Pas homogène pattern en entonnoir
Voir quoi faire quand ecart à l'homo
