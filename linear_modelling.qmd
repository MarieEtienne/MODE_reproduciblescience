...
Soit X une variable binaire à deux modalités : 1 (succès) et 0 (échec).
La probabilité de succès est : $$ P(X = 1) = \pi $$
Alors Y, variable aléatoire correspondant à N tirages de la variable X suit une loi binomiale : 
Y ~ B(N,p)

La fonction de densité de Y est : $$ f(y;\pi)=\binom{N}{y}.\pi^{y}.(1-\pi)^{(N-y)}$$
L'espérance et la variance de Y sont telles que : 
