...
This sounds complicated, **but don't panic** ! In ecology we are lucky to have a great tool, R, which allows us to resolve this type of system. We will see how can R help us with an harder example.

#### Solving 3 equations systems

Use R for study the behaviour of our model as double interest.

-   Verify if our theorical calcul is great

-   If you can't do your calcul help you to have an idear of the behaviour of the model

To show how you can use phaseR and deSolve to solve ODE we will use the SIR we just solve. We hope our calcul is good but we will see. In our presentation we will show what's the most common function you can use not all the parameter you can add. I know you are probably sobbing crying "wHyYyYyy ?" (Who has never dream to know all the difference between the Euler and Runge Kunta order 4 ?) but i let you made you own search.

Lets start by erase all the data in our environnement in order to don't bother us when we work.. Then we load the 3 packages we will use.

-   The most famous library to solve and study ODE is deSolve you can also compute yourself if you want but it will be an harsh task. A great point of deSolve it's this simplicity to use and we hope you would agree with us a the end of this chapter.
...
-   We will also use ggplot to made beautiful graph . It's also a litle more intuative way to create and manipulate plot than using basic ploting system in r.

- rgl is a package to creat 3D plot

As we talk about programmation we have to talk about technical issues you can have. To prevent you to commit this we put during the presentation some technical issues to think of before coding. If you want to do the same things as us you will probably find yourself in some problem don't be afraid R is widely use in the science community you willeasily find people who have defeat the same problem. Don't be ashame to search help !
...
It's time to start modelling! The first step is to creat our fonction who give us the evolution of our population according time. This function to take a step of time, the initial population value, and value of different parameters. The output off this function needs to be a list of the new of our populations.
To have the behaviour of our function we have to use the same hypothesis as in calculation part so we will don't code how the Recover compartiment evoluate (R). If we need the Recover evolution we will use
$$1 = S + I + R \Leftrightarrow R = 1 - S - I $$


Technical consideration: When you build you function you can are free to made some modification BUT the order of the input have to be time, population, parameter value because the ODE function want you to do this!
...
Step 2: study how the sytem evoluate among time

Now we have done the heart of our model let's study it with ODE. It's from deSolve package to solve numerically our model. In order to do this we define to some quantities.
For the time we use the seq function which creat a vector who go from 0 to 600 by 0.01 step. For parameters values we creat a vector in same order that in our function. And value we start form

Then we use ODE to simulate how our population evolve at each time in our vectors. 
We take the output from ODE and manipulate it a little bit to make visualization easier
...
colnames(resolnum) <- c("Times" , "Susceptible" ,"Infectious" )
resolnum$Recovered <- 1-  resolnum$Susceptible + resolnum$Infectious
...
```
Now we have an idear of how our population evoluate trhought time we can try to find the behaviour

Step 3: Equilibrum point and behaviour

Let's study the behavior of our model and its equilibrium points. Once again, we're going to study that S as a function of I First function displays model behavior Second to draw the lines where the models cancel out

```{r}
  flowField(mod_SIR ,xlim=c(-0.1,1.1),ylim=c(-0.1,1.1),parameters = param,add = F)
nullclines(mod_SIR ,xlim=c(-0.1,1.1),ylim=c(-0.1,1.1),parameters = param,points = 100, add = T,add.legend = F,size =2)
```

We trace several trajectories to see how the model behaves
...
cond_init = c(X0, Y0)
  trajectory(mod_SIR ,cond_init,tlim= c(0,50),parameters = param,col = "black",add = T)
}
```

We also use find equilibrum with the this two function. findEquilibrum and stability?

```{r}
findEquilibrium(SIR, y0 = c(0.99,.01),parameters = param )$classification
stability(SIR, y0 = c(0.99,.01),parameters = param)
...
This time our problem is a little bit more complex thna before phase R isn't made to solve 3 equations system so we have th cheat. We can compute our function but unless you a nerd or MODE you don't want to do this. the solution we choose in this paragrph it's to solve system 2 by 2.
...
We create a function with the same limitation.
...
In order to determine the behaviour of our system we should use the ode function to make many graph and try to understand numerically which parameter ar important. In the following example we pass this step and present you 3 cases we can have in our system
...
```{r}
param <- c(r1 = 0.3, r2 = 0.2 , r3 = 0.1, b1 = 0.04, b2 = 0.01, c1 = 0.009, c2 = 0.01)
temps = seq(from = 0 , to = 100, by = 0.01)
depart = c(20 , 10 , 5)
resolnum <- ode(y = depart, times = temps, func = PPS, parms = param)
