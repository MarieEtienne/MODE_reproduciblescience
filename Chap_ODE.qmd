...
X_init<-20    #starting value for x
Y_init<-8     #starting value for y
```
...
ggplot(LV, aes(x = time)) +
  ...
  theme_classic()
```
*** jouer avec les valeurs pour avoir un truc cyclique adéquat
Je vais mentir en attendant : you can see the cyclic nature of this dynamic as explained earlier: the prey thrives when the predator declines, the predator thrives when there's a lot of prey...  
