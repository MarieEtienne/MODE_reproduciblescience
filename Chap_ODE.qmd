...
# Install packages
pkg <- c("knitr", "ggplot2", "tidyverse", "draw", "devtools","deSolve")
...
It's now been ?x?(= number of chapters before ours) chapters since you
plunged into this adventure full of numbers, formulas and other
mathematical oddities (🤢). At the moment, you're probably feeling a
strong need for a change of orientation. I mean, we're doing ecology,
not maths! And raising goats in Auvergne sounds pretty cool...kinds
of...🐐
...
First of all, let's define the framework of the study (= cadre de
l'étude).
...
We could take the problem from the beginning ... or start again from
what we have already done, this time integrating the continuous
dimension of time.
...
# III. 2-equation ODE systems : Lotka Volterra model
...
In ecology, they are mainly used to model dynamics of populations
interacting.\
Generally, if we want to quantify the effect of a species on another
species... What easily comes to mind is that it all depends ob the
...
but eventually exhaust their resources and the population declines. When
the predator population has declined sufficiently, the prey can
...
\*\*\*\* voir avec Laura pour mettre autre chose que "d" en coefficient
pour pas confondre avec les dérivées + voir aussi comment on s'accorde
pour les signes
...
The main distinction to make in those equation is with "x" and "y" :
make sure to recognize that "x" is referring to the prey population (as
...
The initial values for the populations' sizes have been defined. Now, since LV is a dynamic model, evolving over time, and since we do not want our simulation to run endlessly, it'd be a good idea to set a time limit, wouldn't it be? Let's say we want to follow the population dynamics throughout 160 months.  
...
```{r, echo=TRUE}
tend<-160    #how long the simulation is going to run for
...
mathematical setting, we can understand how it works and what is it's
...
>>>>>>> 2388a728633c618d3b69419b4a32c5aea1db4020
...
- has many applications: in mechanics, physics, geometry and biology
- is an equation relating a variable to its derivative(s)
- describes the rate at which a variable varies over time
- has an infinite number of solutions, so it's important to define the initial conditions
