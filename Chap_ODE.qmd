...
#### Résolution sous R d'un système à deux équations

inspiration visuelle interessante : https://rstudio-pubs-static.s3.amazonaws.com/32888_197d1a1896534397b67fb04e0d4899ae.html
...
param <- c(alpha = 0.2, beta = 0.01, gamma = 0.2)
temps = seq(from = 0 , to = 60, by = 0.01)
depart = c(0.999 , 0.001 )
resolnum <- ode(y = depart, times = temps, func = SIR, parms = param)
...
```{r}
  flowField(SIR,xlim=c(-0.1,1.1),ylim=c(-0.1,1.1),parameters = param,add = F)
nullclines(SIR,xlim=c(-0.1,1.1),ylim=c(-0.1,1.1),parameters = param,points = 100, add = T,add.legend = F)
...
cond_init = c(X0, Y0)
  trajectory(SIR,cond_init,tlim= c(0,50),parameters = param,col = i)
}
```

Il existe la fonction findequilibrum qui est ... intéressante...

-   faireplot3D pour comportement modèle dans l'espace
...
presentation librair

The most famous library to solve and study ODE is DeSolve you can also compute yourself if you want but it will be a harsh task. A great point of desolve it's this simplicity tu use and we hope you would agree with us a the end of this chapter. We will also use ggplot to made beautifull. ggplot can be present as more a more intuative way to create and manipulate plot than using basic ploting system in r
...
Ordre important pour t y mais nous verrons après pourquoi
...
Présenation du comportement de l'équation

et fonctionnement ODE

étape de con faire attention à son modele

```{r}
param <- c(a = 0.5 , b = 0.04, c = 0.01 , d = 0.02 , e = 0.04 ,f = 0.1, g = 0.009 )
temps = seq(from = 0 , to = 200, by = 0.01)
depart = c(20 , 10 , 5)
...
ggplot(resolnum10)+
  geom_line(aes(x = Prey, y = Predators),color = "black",size =1.5)+
    theme_classic()

ggplot(resolnum10)+
  geom_line(aes(x = Predators, y = SuperPredators),color = "black",size =1.5)+
    theme_classic()

ggplot(resolnum10)+
  geom_line(aes(x = Prey, y = SuperPredators),color = "black",size =1.5)+
    theme_classic()

plot3d(resolnum[,2],resolnum[,3],resolnum[,4])
```

Maintenant analyse point d'équilibre

Plusieurs fonctions importantes

one présente pas tout argument
