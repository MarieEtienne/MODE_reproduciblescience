...
This sounds complicated, **but don't panic** ! In ecology we are lucky to have a great tool, R, which allows us to resolve this type of system.

### b) Résolution avec R

Dans cette partie nous allons voir comment l'aide des outils disponibles sous R nous pouvons résoudre des systèmes à 3 équations. L'avantage de R par rapport à la partie calculatoire présenté ci dessus c'est qu'elle ne demande pas de capacité de niveau mathématique.

Nous allons commencer par définir notre modèle et
...
Si les méthodes présentés ci dessus semblent magique il faut avoir conscience de plusieurs problèmes. Déjà il faut noter que fait des approximations dans ses calculs. Quand c'est possible il est intéressant de vérifier nos résultats avec la méthode présente avant.
