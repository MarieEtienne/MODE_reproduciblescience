...
La r?gression est une m?thode de mod?lisation empirique d'une variable r?ponse\
(appel?e ici $Y$) en fonction d'une ou de plusieurs variables explicatives ( $X_1,X_2,.,X_n$ avec $n$ le nombre de variables explicatives). Ce sont les r?gressions lin?aires qui sont majoritairement utilis?es dans le domaine des statistiques, principalement pour leur simplicit? d'application. Pla?ons nous donc dans ce monde lin?aire. Le mod?le lin?aire est mod?lis? par :

$$ 
Y_i = \alpha + \sum_{j=1}^{n}b_j \times X{ij} +\epsilon\_i\sim\mathcal{N}(0,,\sigma)
$$ {#eq-1}

Avec $Y_i$ la $i^{?me}$ valeur de la variable $Y$ et $X_{ij}$ la $i^{?me}$ valeur de la $j^{?me}$ variable $X$.

### Test statistique sur les coefficients.

Il faut maintenant s'assurer que les param?tres \$b\$ et $\alpha$ sont diff?rents de 0. Le test par permutation permet de d?finir l'hypoth?se nulle (H0 : $\alpha=0$ et $b = 0$). L'id?e est de permuter les valeurs de

$Y_i$ par rapport aux $X_ij$ ou inversement.
...
Pour chaque permutation, on construit un mod?le lin?aire. L'ensemble des param?tres estim?s ? partir des ?chantillons permut?s forment H0. Le param?tre b est ?stim? par la m?thode des moindres carr?s tels que :

$$
b_i=\frac{\sum(x_i-\bar{x})(y_i-\bar{y})}{\sum(x_i-\bar{x})}
$$

Le param?tre $\alpha$ est estim? par : $$
\alpha=\bar{y}-b_i\bar{x}
$$

```{r}

n_permu <- 1000 # choix du nombre de permutations
...
### coefficients de la regression observ?e

```{r}

# Calcul des coefficients de la regression ? partir des donn?es brutes
obs <- lm(data$Sepal.Length~data$Sepal.Width)$coefficients 

# Distribution de H0 et coefficients observ?s
...
# Calcul des statistiques associ?es au test
...
Le test de mantel (Mantel, 1967) permet d'évaluer la relation entre deux matrices de correspondances mesurées sur les mêmes individus. On note $X$ et $Y$ les deux matrices de correspondances (ou distances). Ici, l'hypothèse nulle décrit l'indépendance des mécanismes qui régissent les distances entre individus. La distribution de H0 s'obtient par permutation des individus entre-eux.

### matrices de correspondances (distances)

Les matrices de distances doivent porter sur les mêmes individus. On peut utiliser le package vegan pour les extraire d'un jeu de données. On utilise ici un jeu de données issu du site de l'UCLA.

```{r}
library(vegan)

# Téléchargement d'un jeu de données et création des matrices de distances
...
ozone.dists %>% # Visualisation d'une matrice de distances
...
Il existe différentes libraries permettant de réaliser le test de Mantel.

```{r}

# En utilisant le package vegan, corrélation de pearson
mantel(xdis = station.dists, ydis = ozone.dists, permutations = 99)
```

```{r}
# En utilisant le package ade4
