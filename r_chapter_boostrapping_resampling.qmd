...
Mantenant que nous avons découvert en détail lors de la partie précédente la méthode de rééchantillonnage par Jacknife, nous allons pouvoir nous pencher sur une seconde méthode de rééchantillonnage : la méthode par Bootstrap. Celle-ci présente le même objectif que précédemment : réaliser des estimations des paramètres descriptifs d'un échantillon et évaluer la précision de ces estimations, pour inférer ces informations à la population réelle. C'est donc de même, une méthode d'inférence statistique par réplication multiple des données à partir du jeu de données initial. En d'autres termes, comme lors de la partie précédente, notre objectif ici est d'obtenir des paramètres d'intérêt à partir de notre échantillon (tels que la moyenne, la médiane ou même le $R^2$ d'une régression et une multitude d'autres) et de construire des intervales de confiance autour de ces derniers.

L'élément clef distinguant la méthode par Jacknife à celle par Bootstrap est qu'à chacune de ses $i$ itérations, nous allons rééchantillonner par tirage aléatoire $n$ éléments avec remise, dans un échantillon intial de $n$ données. Autrement dit, à chaque itération de la méthode, les éléments sont prélevés de l'échantillon initial, puis directement remis à chaque sélection aléatoire. Ceci implique deux différences importantes avec le rééchantillonnage que nous obtenions avec la méthode par Jacknife : i) les nouveaux échantillons sont de même taille $n$ que le jeu de données initiale, ii) un élément donné peut-être présent plusieurs fois dans un nouvel échantillon, ou ne pas l'être du tout.

Une fois ces $i$ échantillons créés par boostrap, les paramètres statistiques souhaités sont alors calculés pour chacun des échantillions. On obtient ainsi une distribution de ces échantillons, composée de $i$ données. Enfin, c'est en analysant cette distribution que nous pourrons estimer la précision et notamment l'intervale de confiance de nos paramètres statistiques.

## Applications

Tentons maintenant d'illustrer ces propos et de leur donner du sens au travers de quelques exemples. Nous allons illustrer comme précédemment différentes applications de la méthode de Bootstrap en fonction des potentiels jeux de données qui peuvent se présenter à vous.

### La base : Calcule de l'intervalle de confiance autour de la moyenne

Comme précédemment réalisé pour illustrer la méthode de Jacknife, nous utiliserons les données de largeur de sépales issues du jeu de données Iris de R base.

```{r}
## Importation des données Iris
dataPetal <- iris$Petal.Width
```
