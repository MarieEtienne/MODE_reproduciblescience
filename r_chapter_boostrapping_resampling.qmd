...
How to do when we have only quantitative variables ?
...
We got the coefficients ! Now we need to make sure that the relationship between the variables are not due to random. In other terms, The way to do that is to verify if the values of the parameters $b$ and $\alpha$ are different from the hypothesis of random. So, we call this hypothesis of random relationship between the variables the null hypothesis (H0). The permutation test is used to define H0. The idea is to permute the values of $Y_i$ with respect to $X_ij$ or vice versa. Here, permutations allow to simulate randoms relationships between variables. 
...
The p value we obtained correspond to the probability of the observation knowing H. Considering a threshold (mainly 0.05) we can determine the significance of the parameters. If the p value is under the threshold, the parameter is significant and vis versa.

Note that you can make permutations tests on all regressions coefficients if $Y_i$ and $X_i$ are interchangeable.
...
The H0 distribution is obtained by permuting the individuals between them. We will use the data set "ozone" from the website UCLA (https://stats.idre.ucla.edu/stat/r/faq/ozone.csv). In this case, the Mantel's test is used to know more about correlations between ozone differences and geographical distances between station. In others terms : Is the ozone variability independent from the geographical variability? 
