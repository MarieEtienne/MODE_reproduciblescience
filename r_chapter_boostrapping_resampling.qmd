...
Overall, Bottstrap's major advantages lie in its broader applicability across various statistical estimations, but which comes with higher computational demands. In contrast, Jackknife might be less versatile, but tends to offer a better computationa efficiency and less biased parameters, making it a favorable choice under certain conditions.

### To go further

This part on the Bootstrap method only covers the most basic application for parameters estimation with explicit script on R for pedagogical reasons. If you end up needing to use Bootstrap, we can highly recommend you to give a look at the `boot` R package. Moreover, this Bootstrap method comes with many derivatives and applications for many usages, so here is a few examples that you might love to delve in if needed:
...
Finally, Bootstrap resampling method can also be used to assess statistical tests, which is what we are going to develop in our next section.
