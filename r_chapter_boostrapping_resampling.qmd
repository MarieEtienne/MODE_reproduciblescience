...
**Authors:** Lucien Ricome, Youna Douchet, Pierre-Alexandre Quitter, Bastien Clémot
...
par(mfrow = c(1, 2))
hist(sub_data$Sepal.Length[sub_data$Species == "versicolor"], col = "skyblue", ylab = "Frequency", xlab = "Sepal Length", main = "Versicolor")
hist(sub_data$Sepal.Length[sub_data$Species == "virginica"], col = "orange", ylab = "Frequency", xlab = "Sepal Length", main = "Virginica", ylim = c(0, 5))
...
$$
\text{Sepal length} \sim \beta + \alpha(\text{Sepal Width}) + \epsilon \sim \mathcal{N}(0, \sigma^2)
$$
...
hist(boot_a, breaks = 40, xlab = TeX("$\\alpha$"), xlim = c(min(null_distribution), max(boot_a)), col = "blue", main = NULL, ylim = c(0,1500))
hist(null_distribution, breaks = 40, add = T, xlab = NULL, ylab = NULL, col = "red")

legend("topright", fill = c("red", "blue"), legend = c("Null distribution", "Bootstrapped distribution"), cex = 1)
```
...
We obtain a p-value of $p < 0.0001$, indicating that the coefficient is significantly greater than chance! Now, let's leverage our combined techniques to calculate the confidence interval of this newly established significant $\alpha$ along with the associated p-value:
