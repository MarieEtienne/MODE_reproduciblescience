...
La régression est une méthode de modélisation empirique d'une variable réponse\
(appelée ici $Y$) en fonction d'une ou de plusieurs variables explicatives ( $X_1,X_2,.,X_n$ avec $n$ le nombre de variables explicatives). Ce sont les régressions linéaires qui sont majoritairement utilisées dans le domaine des statistiques, principalement pour leur simplicité d'application. Plaçons nous donc dans ce monde linéaire. Le modèle linéaire est modélisé par :

$$ 
Y_i = \alpha + \sum_{j=1}^{n}b_j \times X{ij} +\epsilon\_i\sim\mathcal{N}(0,,\sigma)
$$ {#eq-1}

Avec $Y_i$ la $i^{ème}$ valeur de la variable $Y$ et $X_{ij}$ la $i^{ème}$ valeur de la $j^{ème}$ variable $X$.

### Test statistique sur les coefficients.

Il faut maintenant s'assurer que les paramètres \$b\$ et $\alpha$ sont différents de 0. Le test par permutation permet de définir l'hypothèse nulle (H0 : $\alpha=0$ et $b = 0$). L'idée est de permuter les valeurs de
...
Pour chaque permutation, on construit un modèle linéaire. L'ensemble des paramètres estimés à partir des échantillons permutés forment H0. Le paramètre b est éstimé par la méthode des moindres carrés tels que :

$$
b_i=\frac{\sum(x_i-\bar{x})(y_i-\bar{y})}{\sum(x_i-\bar{x})}
$$

Le paramètre $\alpha$ est estimé par : 
$$
...
### coefficients de la regression observée
...
# Calcul des statistiques associées au test
...
Il existe différents packages permettant de réaliser le test de Mantel. Par exemple le package ade4 
...
Le package vegan permet d'utiliser différentes statistiques. Dans notre cas nous avons choisis la correlation de Pearson.
