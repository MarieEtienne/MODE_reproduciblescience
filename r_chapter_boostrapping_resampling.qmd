...
Here we'll test using permutation test the correlation between the age and the circumference of the tree. Let's plot this relation :

```{r}
# RECODING FACTORS LEVELS
Orange$Tree <- as.factor(as.numeric(Orange$Tree))

# VISUALIZE AGE ~ CIRCUMFERENCE
...
According to the visualization, we clearly expect a strong correlation between both variables. Let's follow the previous step to perform the permutation test.

1.  **Formulate null and alternative hypotheses**

    **H1** : the age and the circumference of the tree are correlated. **H0** : the age and the circumference are not correlated

2.  **Determine and calculate the test statistic from the observed data**

    We'll use the coefficient of correlation of Spearman as the test statistic for $P_{obs}$ :

    ```{r}
    p_obs <- cor(Orange$age, Orange$circumference, method = 'spearman')
    p_obs
    ```

3.  **Generate a large amount of permutations on independent unit of the data and calculate the test statistic for each permutation**

    Considering the paired status of the data, we will shuffle randomly the values of the variable `age` **within** each individual to construct the null hypothesis.
    ...
    for (i in 1:iteration)
    {
      ...
      permuted_data <- 
        Orange %>%
        group_by(Tree) %>%
        mutate(age = age[sample(n())])  # randomly shuffle age values within individual levels

      # --- Difference of means of both species under h0
      p_h0 <- cor(permuted_data$age, permuted_data$circumference, method = "spearman")
...
4.  **Compare the observed statistic to the null distribution of test statistics**
    ...
    abline(v = p_obs, lwd = 1.5, col = "blue", lty = 2)
    text(x = .7, y = 80, labels = TeX("$P_{obs}$"), col = "blue")
    ```
...
5.  **Conclude on accepting or rejecting the null hypothesis**
    ...
    As $p < 0.05$, we reject the null hypothesis stating the age is not correlated with the circumference of the orange tree.
