...
How to do when you have only quantitatives variables ?
...
You got the coefficients ! Now we need to make sure that the relationship between the variables are not due to random. In other terms, 
The way to do that is to verify that the values of the parameters $b$ and $\alpha$ are different from the hypothesis of random. So, we call this hypothesis of random relationship between the variables the null hypothesis (H0). The permutation test is used to define H0. The idea is to permute the values of $Y_i$ with respect to $X_ij$ or vice versa. Here, permutations allow to simulate randoms relationships between variables. 

Here an example of one permutation of the $Y_i$ from the $X_i$.
...
The p value we obtained correspond to the probability that the observations knowing H. Considering a threshold (mainly 0.05) we can determine the significant of the parameters. If the p value is under the threshold, the parameter is significant and vis versa.

Note that you can make permutations tests on all regressions coefficients when $Y_i$ are interchangeable.

## Mantel test

### Uses

The Mantel test (Mantel, 1967) is used to evaluate the relationship between two matrices of correspondences measured on the same individuals. The two correspondence (distances) matrices are denoted by $X$ and $Y$. Here, the null hypothesis describes the independence of the mechanisms governing the distances between individuals. The Mantel test is mainly used for test correlations between genetics or geographical distances.
The H0 distribution is obtained by permuting the individuals between them. We will use the data set "ozone" from the website UCLA (https://stats.idre.ucla.edu/stat/r/faq/ozone.csv). In this case, the Mantel's test is used to know more about correlations between ozone differences between station and geographical distances between station. In other terms : Is the ozone variability independent from the geographical variability? 
...
The distances matrix should have the same shape than the table x. Now we can apply the Mantel test on our to matrix.

### Mantel's test apllication

There are some packages available to perform the Mantel test. For example, the ade4 package with the "mantel.rtest" function as follow.
...
```
The coefficient of correlation observed is -0.2061116 and the p value is 0.6793207. If we put the threshold of significance on 0.05, the correlation is not significant here !

An other example of package is the vegan package which allows to use different correlation statistics. In our case we chose the Pearson correlation. 
...
```
Using this package, the results are similar, well! The p value is indicate after the term "Significance:" and the correlation coefficient follow the term "Mantel statistic r:". Note that in our case, the number of possible permutations is limited. We should be aware on the length of our data set. 